...
<main class="p-6 text-white bg-gray-900 min-h-screen max-w-6xl mx-auto">
	...
	<div class="fixed top-6 right-6 w-80 max-h-[85vh] p-4 rounded-lg bg-gray-800/80 border border-gray-700 shadow-md backdrop-blur-md overflow-y-auto cursor-move">
		...
		<ul id="attachmentPool" class="drag-and-drop__items text-xs">
			{#each allAttachments as att (att.id)}
				<li class="drag-and-drop__item" data-id={att.id}><strong>{att.index + 1}.</strong> {att.text || att.raw}</li>
			{/each}
		</ul>
	</div>
